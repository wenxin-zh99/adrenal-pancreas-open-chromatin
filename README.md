...
To identify biological processes associated with species-specific open chromatin regions, we performed GO Biological Process enrichment analysis using `rGREAT`.
...
We used relevant peak sets rather than the whole genome as background, because the goal was to compare functional enrichment within experimentally observed regulatory regions.

### Main script
- `scripts/GO.R`

### Results
Task 4 output files are stored in:
...
- dotplot visualization of enriched GO terms
